# -*- coding: utf-8 -*-
"""my_project_nlp_baseline_v1.ipynb

Automatically generated by Colaboratory.

Original file is located at
	https://colab.research.google.com/drive/1rxrD3HLusfoinrxURhUEN4fNhOwPb6Wj
"""
		...
		if current_line[0] == new_idx:
			...
			if cnt_root == 1 and check_projectivity(new_current_tokens) == True: #if current tree is projective
				...
				for i in range(len(configuration_state)):
					features , labels = get_features_labels(configuration_state[i],gold_transitions[i], tokens_dict, POS_tags)
...
def convert_to_training_data():
	...
	for feature in tqdm(feature_counts):
		if feature_counts[ feature ] > 5 and feature_vocabulary.get(feature) is None:
...
trained_model = train(X_train, y_train, 'lr_el')

def action_to_tree(test_dependency_tree,prediction_probabs,test_buffer,test_stack,test_arcs):
...
def evaluate_oracles_and_predictions(test_tokens, model):
	...
	while len(test_buffer) > 0:
		...
		test_features, _ = get_features_labels(current_config,None,test_tokens_dict,test_POS_tags)
		...
		action_to_tree(test_dependency_tree,prediction_probabs,test_buffer,test_stack,test_arcs)
